window.uidata = {
    menu: [{
        ...
        {
            title: "Portfolio",
            url: "http://balogungift.ga/index.php"
        },
        {
            title: "Github Page",
    ...
    config: {
        ...
        subtitle: "",
        logo: "http://balogungift.ga/images/logo.png"
    }
}
...
function onWinLoad() {
    ...
    window.uidata.menu.forEach(elm => {
        $('#menu-holder').prepend(`<li> <a href="${elm.url}">${elm.title}</a> </li>`);
        $('#hidden-menu').append(`<li> <a href="${elm.url}">${elm.title}</a> </li>`);
    })
...
// Show amd Hide menu
document.querySelector('.top-menu .menu-title').addEventListener('click', elm => {
    let btn = elm.target;
